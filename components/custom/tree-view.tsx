...
import SubCategoryAction from "@/modules/admin/ui/components/manages/categories/button/sub-category-action";
            ...
            >
                {renderItem ? (
                    ...
                    <div className="flex w-full justify-between items-center">
                        ...
                        <SubCategoryAction isLastLevel={true} parentId={item.id} level={level} parentName={item.name} />
                    </div>
                )}
            </div>
        );
...
>(({ className, children, isOpen, ...props }, ref) => (
    <AccordionPrimitive.Header>
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn("flex flex-1 w-full items-center py-2 transition-all group", className)}
            {...props}
        >
            <ChevronRight
                className={cn(
